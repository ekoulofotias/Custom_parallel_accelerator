...
module vector_alu (
	...
	output wire [`VEC_SIZE-1:0] CARRY_OUT,		// Flag Carry
	output wire [`VEC_SIZE-1:0] OVERFLOW_OUT,	// Flag Overflow

	// CONTROL BUS
	input wire [3:0] OPCODE,					// VOP_ADD_SUB, VOP_BITWISE, VOP_SHIFT, VOP_HADAMARD, VOP_DOT
	input wire [2:0] SUB_OP,					// bitwise Sub-Op, Add/Sub flag, shift direction
	...
	wire [`TOTAL_SIZE-1:0] res_hadamard;

	// Add / Subtract Unit
	vector_add_sub ADD_SUB(.S(res_add_sub), .C(CARRY_OUT), .OV(OVERFLOW_OUT), .A(VEC_A), .B(VEC_B), .OP(SUB_OP[0]));
	...
	always @(*) begin
		case (OPCODE)
			`VOP_ADD_SUB 	: ALU_OUT = res_add_sub;
