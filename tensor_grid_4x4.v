...
module tensor_grid_4x4 (
	...
	// Tiles of matrix C (2x2)
	input wire [`BIT_SIZE*2-1:0] C00_00, C00_01, C00_10, C00_11,
	input wire [`BIT_SIZE*2-1:0] C01_00, C01_01, C01_10, C01_11,
	input wire [`BIT_SIZE*2-1:0] C10_00, C10_01, C10_10, C10_11,
	input wire [`BIT_SIZE*2-1:0] C11_00, C11_01, C11_10, C11_11
);

	// Constant zero wire for unused C inputs to avoid double addition
	wire [`BIT_SIZE*2-1:0] ZERO_TILE = {`BIT_SIZE*2{1'b0}};
	...
	// TC00_B (C set to 0 so it acts purely as partial product multiplier)
	tensor_core_2x2 TC00_B (
		...
		.C00(ZERO_TILE), .C01(ZERO_TILE), .C10(ZERO_TILE), .C11(ZERO_TILE)
	);
	...
	// TC00_A (Passes Matrix C tiles here so they get accumulated properly)
	tensor_core_2x2 TC00_A (
		...
		.C00(C00_00), .C01(C00_01), .C10(C00_10), .C11(C00_11)
	);
	...
	tensor_core_2x2 TC01_A (
		...
		.C00(C01_00), .C01(C01_01), .C10(C01_10), .C11(C01_11)
	);
	...
	tensor_core_2x2 TC10_A (
		...
		.B00(B00_00), .B01(B00_01), .B10(B00_10), .B11(B00_11),
		.C00(C10_00), .C01(C10_01), .C10(C10_10), .C11(C10_11)
	);
	...
	tensor_core_2x2 TC11_A (
		...
		.A00(A10_00), .A01(A10_01), .A10(A10_10), .A11(A10_11),
		.B00(B01_00), .B01(B01_01), .B10(B01_10), .B11(B01_11),
		.C00(C11_00), .C01(C11_01), .C10(C11_10), .C11(C11_11)
	);

	// Combined tile additions
	assign D00_00 = $signed(P00_A_00) + $signed(P00_B_00);
	assign D00_01 = $signed(P00_A_01) + $signed(P00_B_01);
	assign D00_10 = $signed(P00_A_10) + $signed(P00_B_10);
	assign D00_11 = $signed(P00_A_11) + $signed(P00_B_11);

	assign D01_00 = $signed(P01_A_00) + $signed(P01_B_00);
	assign D01_01 = $signed(P01_A_01) + $signed(P01_B_01);
	assign D01_10 = $signed(P01_A_10) + $signed(P01_B_10);
	assign D01_11 = $signed(P01_A_11) + $signed(P01_B_11);

	assign D10_00 = $signed(P10_A_00) + $signed(P10_B_00);
	assign D10_01 = $signed(P10_A_01) + $signed(P10_B_01);
	assign D10_10 = $signed(P10_A_10) + $signed(P10_B_10);
	assign D10_11 = $signed(P10_A_11) + $signed(P10_B_11);

	assign D11_00 = $signed(P11_A_00) + $signed(P11_B_00);
	assign D11_01 = $signed(P11_A_01) + $signed(P11_B_01);
	assign D11_10 = $signed(P11_A_10) + $signed(P11_B_10);
	assign D11_11 = $signed(P11_A_11) + $signed(P11_B_11);
